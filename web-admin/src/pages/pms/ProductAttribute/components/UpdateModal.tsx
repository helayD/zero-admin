...
const UpdateModal: React.FC<UpdateModalProps> = (props) => {
  ...
  const { onSubmit, onCancel, updateVisible, currentData, scope } = props;
  ...
  return (
    <Modal forceRender destroyOnClose title="编辑" open={updateVisible} okText="保存" onOk={() => form.submit()} onCancel={onCancel}>
      <Form {...formLayout} form={form} onFinish={(values) => onSubmit?.(values as ProductAttributeListItem)}>
        ...
        <FormItem name="inputType" label="输入类型" rules={[{ required: true, message: '请选择输入类型!' }]}>
          <Radio.Group><Radio value={1}>手动输入</Radio><Radio value={2}>单选</Radio><Radio value={3}>多选</Radio></Radio.Group>
        </FormItem>
        <FormItem name="valueType" label="值类型" rules={[{ required: true, message: '请选择值类型!' }]}>
          <Radio.Group><Radio value={1}>文本</Radio><Radio value={2}>数字</Radio><Radio value={3}>日期</Radio></Radio.Group>
        </FormItem>
