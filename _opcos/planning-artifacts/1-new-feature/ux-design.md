...
**Revision Focus:** 对齐最新 PRD 的移动端 App 召回、弱网、权限、升级提示与数字卡片抽赏链路要求。
...
- **数字卡片到账成功**：抽卡后能立即看到中奖结果、唯一编号与链上 token 状态，并在处理中或补偿中保持可解释，是数字资产营销是否可信的关键节点。
...
- 对数字卡片场景优先优化“已中奖但未完全到账”的解释力，避免把处理中、补偿中、冻结中误渲染为失败或成功。
...
**Purpose:** 在“我的数字卡片”列表中稳定展示卡面、唯一编号、所属活动、稀有度和链上状态。  
...
- 商城端自定义组件优先服务于详情页、订单确认页、订单详情、个人中心、数字卡片资产页以及启动恢复链路。
- 数字卡片相关组件必须默认采用服务端确认优先模式，中奖结果、唯一编号、token 状态和补偿说明由聚合接口统一返回。
...
- **Digital Asset Status**：数字卡片统一使用“待实名 / 可参与 / 未中奖 / 已中奖待到账 / 链上处理中 / 补偿中 / 已到账 / 已冻结 / 已下线展示”这组状态词，不允许页面各自发明近义词。
- **Asset Ownership Proof**：只要页面声称“已获得卡片”，就必须同时给出卡面、唯一编号或待生成说明、所属活动和当前 token 状态四类信息。
- **Ops Audit Flow**：后台数字卡片工作台默认采用“固定筛选区 + 列表摘要 + 右侧详情 / 抽屉 + 危险动作确认”模式，重试、冻结、人工复核不得隐藏在二级页面深处。
...
- 数字卡片页面不允许客户端自行推断“已到账”；所有到账、补偿、冻结语义都必须以后端聚合状态为准。
- 后台数字卡片模块优先采用工作台式信息架构，活动配置、发卡任务和审计检索保持同一套状态标签与处置动作语义。
