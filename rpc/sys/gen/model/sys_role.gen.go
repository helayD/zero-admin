...
type SysRole struct {
	...
	ScopeType  string     `gorm:"column:scope_type;not null;default:platform;comment:作用域类型（platform:平台级 tenant:租户级 merchant:商户级）" json:"scope_type"` // 作用域类型
	PlatformID int64      `gorm:"column:platform_id;not null;default:1;comment:平台ID" json:"platform_id"`                                             // 平台ID
	TenantID   int64      `gorm:"column:tenant_id;not null;default:0;comment:租户ID（0表示非租户级）" json:"tenant_id"`                                        // 租户ID
	MerchantID int64      `gorm:"column:merchant_id;not null;default:0;comment:商户ID（0表示非商户级）" json:"merchant_id"`                                    // 商户ID
	IsAdmin    int32      `gorm:"column:is_admin;not null;default:0;comment:是否超级管理员角色（1:是 0:否）" json:"is_admin"`                                     // 是否超级管理员角色
	DataScope  int32      `gorm:"column:data_scope;not null;default:1;comment:数据范围（1：全部数据权限 2：自定数据权限 3：本部门数据权限 4：本部门及以下数据权限）" json:"data_scope"`     // 数据范围
