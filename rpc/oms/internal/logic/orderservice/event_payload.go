...
type OrderEventPayload struct {
	...
	Source     string                 `json:"source,omitempty"` // 来源：user-用户主动,timeout-超时自动
	Data       map[string]interface{} `json:"data,omitempty"`   // 业务数据上下文
}

// sendOrderEvent 统一订单域事件发送入口
// 替换原有的直接拼 JSON payload 模式
func sendOrderEvent(ctx context.Context, svcCtx *svc.ServiceContext, queue, routingKey, action, source string, orderID int64, current pkgscope.GovernanceScope, actorID int64, data map[string]interface{}) {
