...
func (l *DisableTenantLogic) DisableTenant(req *types.ChangeTenantStatusReq) (resp *types.BaseResp, err error) {
	...
	_, err = l.svcCtx.TenantService.DisableTenant(l.ctx, &sysclient.ChangeTenantStatusReq{
		Ids:          req.Ids,
	...
	if err != nil {
		logc.Errorf(l.ctx, "停用租户失败, 参数: %+v, 异常: %s", req, err.Error())
