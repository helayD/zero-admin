...
func newConsumerMintTestDB(t *testing.T) *gorm.DB {
	t.Helper()
	...
	if err := db.Exec(`
		...
		VALUES
			(1, 2001, 3001, 11, 21, 'req-consumer', 'trace-consumer', '{"realNameStatus":"verified"}', 1, 0)
	`).Error; err != nil {
	...
	if err := db.Exec(`
		...
		VALUES
			(1, 1, 10, 88, 2001, 3001, 1, 'req-consumer', 'trace-consumer', 'platform:1,tenant:10,merchant:88', 11, 21, 'SSR', 'CARD-CONSUMER', 'asset_created', 'mint_pending', '', '', 0, 0, 0)
	`).Error; err != nil {
...
func newConsumerMintService(t *testing.T) (*digitalcardmint.Service, int64, *gorm.DB) {
	t.Helper()

	db := newConsumerMintTestDB(t)
	service := digitalcardmint.NewService(db, nil, antchain.NewMockClient())
	service.Now = func() time.Time {
		return time.Date(2026, 4, 18, 11, 0, 0, 0, time.Local)
	}
	...
	return service, taskID, db
}

func TestMintRequestedExecutesTaskAndSupportsReplay(t *testing.T) {
	service, taskID, db := newConsumerMintService(t)

	body, err := json.Marshal(digitalcardmint.MintRequestedEvent{
		EventName:       digitalcardmint.EventNameMintRequested,
		TaskID:          taskID,
		AssetInstanceID: 1,
	...
	if err := MintRequested(context.Background(), body, service); err != nil {
		t.Fatalf("first MintRequested returned error: %v", err)
	}
	if err := MintRequested(context.Background(), body, service); err != nil {
		t.Fatalf("second MintRequested returned error: %v", err)
	}
	...
	if task.TaskStatus != digitalcardmint.TaskStatusSucceeded || task.TokenID == "" {
		t.Fatalf("expected task succeeded after replay, got %+v", task)
	}

	var logCount int64
	if err := db.Table(digitalcardmint.CardAssetLogRow{}.TableName()).Count(&logCount).Error; err != nil {
		t.Fatalf("count logs failed: %v", err)
	}
	if logCount != 2 {
		t.Fatalf("expected 2 logs after replay-safe consume, got %d", logCount)
	}
}

func TestMintRequestedReturnsErrorOnInvalidPayload(t *testing.T) {
	service, _, _ := newConsumerMintService(t)

	if err := MintRequested(context.Background(), []byte(`{"taskId":0}`), service); err == nil {
		t.Fatal("expected invalid task id to return error")
	}
}
